...
export function startFirstGame(pageLeftSide: any, pageRightSide: any, nickName: string) {
    ...
    if (keyPosition[0] < ((pageRightSide.offsetWidth / 100) * 40))
        keyPosition[0] = ((pageRightSide.offsetWidth / 100) * 40 + keyPosition[0]) | 0;
    ...
    let mouseMove$ = zip(combineMouseXYPosition$('mousemove')).subscribe(e => {
        ...
        else if ((pointerPosition[0].x > keyPosition[0] - 150 && pointerPosition[0].x < keyPosition[0] + 150)
            && (pointerPosition[0].y > keyPosition[1] - 150 && pointerPosition[0].y < keyPosition[1] + 150)) {
        ...
        else if ((pointerPosition[0].x > keyPosition[0] - 350 && pointerPosition[0].x < keyPosition[0] + 350)
            && (pointerPosition[0].y > keyPosition[1] - 350 && pointerPosition[0].y < keyPosition[1] + 350)) {
            yellowCircle.style.backgroundColor = yellow;
            blueCircle.style.backgroundColor = white;
            redCircle.style.backgroundColor = red;
            keyButtonToFind.style.display = "none";
